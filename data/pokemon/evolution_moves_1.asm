SECTION "Evolution Moves 1", ROMX

EvolutionMovePointers1:
	dw NoEvolutionMoves1         ; BULBASAUR
	dw NoEvolutionMoves1         ; IVYSAUR
	dw VenusaurEvolutionMoves    ; VENUSAUR
	dw NoEvolutionMoves1         ; CHARMANDER
	dw NoEvolutionMoves1         ; CHARMELEON
	dw CharizardEvolutionMoves   ; CHARIZARD
	dw NoEvolutionMoves1         ; SQUIRTLE
	dw NoEvolutionMoves1         ; WARTORTLE
	dw BlastoiseEvolutionMoves   ; BLASTOISE
	dw NoEvolutionMoves1         ; CATERPIE
	dw MetapodEvolutionMoves     ; METAPOD
	dw ButterfreeEvolutionMoves  ; BUTTERFREE
	dw NoEvolutionMoves1         ; WEEDLE
	dw KakunaEvolutionMoves      ; KAKUNA
	dw BeedrillEvolutionMoves    ; BEEDRILL
	dw NoEvolutionMoves1         ; PIDGEY
	dw NoEvolutionMoves1         ; PIDGEOTTO
	dw NoEvolutionMoves1         ; PIDGEOT
	dw NoEvolutionMoves1         ; RATTATA_A
	dw RaticateAEvolutionMoves   ; RATICATE_A
	dw NoEvolutionMoves1         ; PIKACHU
	dw RaichuAEvolutionMoves     ; RAICHU_A
	dw NoEvolutionMoves1         ; SANDSHREW_A
	dw SandslashAEvolutionMoves  ; SANDSLASH_A
	dw NoEvolutionMoves1         ; NIDORAN_F
	dw NoEvolutionMoves1         ; NIDORINA
	dw NidoqueenEvolutionMoves   ; NIDOQUEEN
	dw NoEvolutionMoves1         ; NIDORAN_M
	dw NoEvolutionMoves1         ; NIDORINO
	dw NidokingEvolutionMoves    ; NIDOKING
	dw NoEvolutionMoves1         ; CLEFAIRY
	dw NoEvolutionMoves1         ; CLEFABLE
	dw NoEvolutionMoves1         ; VULPIX_A
	dw NinetalesAEvolutionMoves  ; NINETALES_A
	dw NoEvolutionMoves1         ; JIGGLYPUFF
	dw NoEvolutionMoves1         ; WIGGLYTUFF
	dw NoEvolutionMoves1         ; ZUBAT
	dw GolbatEvolutionMoves      ; GOLBAT
	dw NoEvolutionMoves1         ; ODDISH
	dw NoEvolutionMoves1         ; GLOOM
	dw VileplumeEvolutionMoves   ; VILEPLUME
	dw NoEvolutionMoves1         ; PARAS
	dw ParasectEvolutionMoves    ; PARASECT
	dw NoEvolutionMoves1         ; VENONAT
	dw VenomothEvolutionMoves    ; VENOMOTH
	dw NoEvolutionMoves1         ; MEOWTH_G
	dw NoEvolutionMoves1         ; PSYDUCK
	dw GolduckEvolutionMoves     ; GOLDUCK
	dw NoEvolutionMoves1         ; MANKEY
	dw PrimeapeEvolutionMoves    ; PRIMEAPE
	dw NoEvolutionMoves1         ; GROWLITHE
	dw ArcanineEvolutionMoves    ; ARCANINE
	dw NoEvolutionMoves1         ; ABRA
	dw KadabraEvolutionMoves     ; KADABRA
	dw NoEvolutionMoves1         ; ALAKAZAM
	dw NoEvolutionMoves1         ; MACHOP
	dw NoEvolutionMoves1         ; MACHOKE
	dw NoEvolutionMoves1         ; MACHAMP
	dw NoEvolutionMoves1         ; BELLSPROUT
	dw NoEvolutionMoves1         ; WEEPINBELL
	dw VictreebelEvolutionMoves  ; VICTREEBEL
	dw NoEvolutionMoves1         ; TENTACOOL
	dw TentacruelEvolutionMoves  ; TENTACRUEL
	dw NoEvolutionMoves1         ; GEODUDE
	dw NoEvolutionMoves1         ; GRAVELER
	dw GolemEvolutionMoves       ; GOLEM
	dw NoEvolutionMoves1         ; PONYTA
	dw RapidashEvolutionMoves    ; RAPIDASH
	dw NoEvolutionMoves1         ; SLOWPOKE
	dw SlowbroEvolutionMoves     ; SLOWBRO
	dw NoEvolutionMoves1         ; MAGNEMITE
	dw MagnetonEvolutionMoves    ; MAGNETON
	dw NoEvolutionMoves1         ; FARFETCH_D
	dw NoEvolutionMoves1         ; SEEL
	dw DewgongEvolutionMoves     ; DEWGONG
	dw NoEvolutionMoves1         ; GRIMER
	dw MukEvolutionMoves         ; MUK
	dw NoEvolutionMoves1         ; SHELLDER
	dw CloysterEvolutionMoves    ; CLOYSTER
	dw NoEvolutionMoves1         ; GASTLY
	dw HaunterEvolutionMoves     ; HAUNTER
	dw NoEvolutionMoves1         ; GENGAR
	dw NoEvolutionMoves1         ; ONIX
	dw NoEvolutionMoves1         ; DROWZEE
	dw HypnoEvolutionMoves       ; HYPNO
	dw NoEvolutionMoves1         ; KRABBY
	dw KinglerEvolutionMoves     ; KINGLER
	dw NoEvolutionMoves1         ; EXEGGCUTE
	dw ExeggutorAEvolutionMoves  ; EXEGGUTOR_A
	dw NoEvolutionMoves1         ; CUBONE
	dw NoEvolutionMoves1         ; MAROWAK
	dw NoEvolutionMoves1         ; LICKITUNG
	dw NoEvolutionMoves1         ; KOFFING
	dw WeezingGEvolutionMoves    ; WEEZING_G
	dw NoEvolutionMoves1         ; CHANSEY
	dw TangelaEvolutionMoves     ; TANGELA
	dw NoEvolutionMoves1         ; KANGASKHAN
	dw NoEvolutionMoves1         ; HORSEA
	dw NoEvolutionMoves1         ; SEADRA
	dw NoEvolutionMoves1         ; GOLDEEN
	dw NoEvolutionMoves1         ; SEAKING
	dw NoEvolutionMoves1         ; STARYU
	dw NoEvolutionMoves1         ; STARMIE
	dw MrMimeGEvolutionMoves     ; MR__MIME_G
	dw NoEvolutionMoves1         ; SCYTHER
	dw NoEvolutionMoves1         ; ELECTABUZZ
	dw NoEvolutionMoves1         ; PINSIR
	dw NoEvolutionMoves1         ; TAUROS
	dw NoEvolutionMoves1         ; MAGIKARP
	dw GyaradosEvolutionMoves    ; GYARADOS
	dw NoEvolutionMoves1         ; LAPRAS
	dw NoEvolutionMoves1         ; DITTO
	dw NoEvolutionMoves1         ; EEVEE   
	dw VaporeonEvolutionMoves    ; VAPOREON
	dw JolteonEvolutionMoves     ; JOLTEON 
	dw FlareonEvolutionMoves     ; FLAREON 
	dw NoEvolutionMoves1         ; PORYGON
	dw NoEvolutionMoves1         ; OMANYTE
	dw OmastarEvolutionMoves     ; OMASTAR
	dw NoEvolutionMoves1         ; KABUTO
	dw KabutopsEvolutionMoves    ; KABUTOPS
	dw NoEvolutionMoves1         ; AERODACTYL
	dw NoEvolutionMoves1         ; SNORLAX
	dw NoEvolutionMoves1         ; ARTICUNO_G
	dw NoEvolutionMoves1         ; ZAPDOS_G
	dw NoEvolutionMoves1         ; MOLTRES_G
	dw NoEvolutionMoves1         ; DRATINI
	dw NoEvolutionMoves1         ; DRAGONAIR
	dw DragoniteEvolutionMoves   ; DRAGONITE
	dw NoEvolutionMoves1         ; MEWTWO
	dw NoEvolutionMoves1         ; MEW

VenusaurEvolutionMoves:
VileplumeEvolutionMoves:
	dw PETAL_STORM
	db 0 ; no more evolution moves

CharizardEvolutionMoves:
	dw AIR_SLASH
	db 0 ; no more evolution moves

BlastoiseEvolutionMoves:
	dw FLASH_CANNON
	db 0 ; no more evolution moves

MetapodEvolutionMoves:
KakunaEvolutionMoves:
	dw HARDEN
	db 0 ; no more evolution moves

ButterfreeEvolutionMoves:
VenomothEvolutionMoves:
	dw GUST
	db 0 ; no more evolution moves

BeedrillEvolutionMoves:
	dw TWINEEDLE
	db 0 ; no more evolution moves

RaticateAEvolutionMoves:
	dw SCARY_FACE
	db 0 ; no more evolution moves

RaichuAEvolutionMoves:
	dw PSYCHIC
	db 0 ; no more evolution moves

SandslashAEvolutionMoves:
CloysterEvolutionMoves:
	dw ICICLE_SPEAR
	db 0 ; no more evolution moves

NidoqueenEvolutionMoves:
	dw SUPERPOWER
	db 0 ; no more evolution moves

NidokingEvolutionMoves:
	dw MEGAHORN
	db 0 ; no more evolution moves

NinetalesAEvolutionMoves:
	dw DAZZLING_GLEAM
	db 0 ; no more evolution moves

GolbatEvolutionMoves:
	dw SCREECH
	db 0 ; no more evolution moves

ParasectEvolutionMoves:
	dw CROSS_POISON
	db 0 ; no more evolution moves

GolduckEvolutionMoves:
	dw AQUA_JET
	db 0 ; no more evolution moves

PrimeapeEvolutionMoves:
	dw RAGE
	db 0 ; no more evolution moves

ArcanineEvolutionMoves:
	dw EXTREMESPEED
	db 0 ; no more evolution moves

KadabraEvolutionMoves:
	dw CONFUSION
	db 0 ; no more evolution moves

VictreebelEvolutionMoves:
	dw LEAF_TORNADO
	db 0 ; no more evolution moves

TentacruelEvolutionMoves:
	dw REFLECT_TYPE
	db 0 ; no more evolution moves

GolemEvolutionMoves:
	dw STEAMROLLER
	db 0 ; no more evolution moves

RapidashEvolutionMoves:
	dw SMART_STRIKE
	db 0 ; no more evolution moves

SlowbroEvolutionMoves:
	dw WITHDRAW
	db 0 ; no more evolution moves

MagnetonEvolutionMoves:
	dw TRI_ATTACK
	db 0 ; no more evolution moves

DewgongEvolutionMoves:
	dw SHEER_COLD
	db 0 ; no more evolution moves

MukEvolutionMoves:
	dw VENOM_DRENCH
	db 0 ; no more evolution moves

HaunterEvolutionMoves:
	dw SHADOW_PUNCH
	db 0 ; no more evolution moves

HypnoEvolutionMoves:
	dw NIGHTMARE
	db 0 ; no more evolution moves

KinglerEvolutionMoves:
	dw HAMMER_ARM
	db 0 ; no more evolution moves

ExeggutorAEvolutionMoves:
	dw DRAGONHAMMER
	db 0 ; no more evolution moves

WeezingGEvolutionMoves:
	dw DOUBLE_HIT
	dw STRANGE_STEAM
	db 0 ; no more evolution moves

TangelaEvolutionMoves:
	dw INGRAIN
	db 0 ; no more evolution moves

MrMimeGEvolutionMoves:
	dw ICE_SHARD
	db 0 ; no more evolution moves

GyaradosEvolutionMoves:
	dw BITE
	db 0 ; no more evolution moves

VaporeonEvolutionMoves:
	dw WATER_GUN
	db 0 ; no more evolution moves

JolteonEvolutionMoves:
	dw THUNDERSHOCK
	db 0 ; no more evolution moves

FlareonEvolutionMoves:
	dw EMBER
	db 0 ; no more evolution moves

OmastarEvolutionMoves:
	dw SPIKE_CANNON
	db 0 ; no more evolution moves

KabutopsEvolutionMoves:
	dw SLASH
	db 0 ; no more evolution moves

DragoniteEvolutionMoves:
	dw WING_ATTACK
	dw HURRICANE
	db 0 ; no more evolution moves

NoEvolutionMoves1:
	db 0 ; no more evolution moves
