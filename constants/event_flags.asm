...
; Johto story events
	...
	const EVENT_GOT_FIRE_STARTER_FROM_ELM
	const EVENT_GOT_WATER_STARTER_FROM_ELM
	const EVENT_GOT_GRASS_STARTER_FROM_ELM
...
; Johto hidden items
	...
	const EVENT_082
	const EVENT_083
	const EVENT_NATIONAL_PARK_HIDDEN_FULL_HEAL_1
...
; Kanto hidden items
	...
	const EVENT_ROUTE_4_HIDDEN_HP_UP
	...
	const EVENT_ROUTE_25_HIDDEN_REVIVE
...
; Random starter is set once examined
	const EVENT_CHARMANDER_IS_FIRE_STARTER
	const EVENT_CYNDAQUIL_IS_FIRE_STARTER
	const EVENT_TORCHIC_IS_FIRE_STARTER
	const EVENT_CHIMCHAR_IS_FIRE_STARTER
	const EVENT_TEPIG_IS_FIRE_STARTER
	const EVENT_FENNEKIN_IS_FIRE_STARTER
	const EVENT_FUECOCO_IS_FIRE_STARTER
	const EVENT_SQUIRTLE_IS_WATER_STARTER
	const EVENT_TOTODILE_IS_WATER_STARTER
	const EVENT_MUDKIP_IS_WATER_STARTER
	const EVENT_PIPLUP_IS_WATER_STARTER
	const EVENT_OSHAWOTT_IS_WATER_STARTER
	const EVENT_FROAKIE_IS_WATER_STARTER
	const EVENT_BULBASAUR_IS_GRASS_STARTER
	const EVENT_CHIKORITA_IS_GRASS_STARTER
	const EVENT_TREECKO_IS_GRASS_STARTER
	const EVENT_TURTWIG_IS_GRASS_STARTER
	const EVENT_SNIVY_IS_GRASS_STARTER
	const EVENT_CHESPIN_IS_GRASS_STARTER
	const EVENT_ROWLET_IS_GRASS_STARTER
; New trainers
	...
	const EVENT_BEAT_FIREBREATHER_CARY
	const EVENT_BEAT_SUPER_NERD_LOWELL
	const EVENT_BEAT_SUPER_NERD_WALDO
	const EVENT_BEAT_SCIENTIST_DANIELLE
	const EVENT_BEAT_SCIENTIST_LINDE
	const EVENT_BEAT_SCIENTIST_MERLE
	...
	const EVENT_BEAT_COOLTRAINERF_ELAN
	const EVENT_BEAT_COOLTRAINERM_IDA
; New Johto items
	const EVENT_ROUTE_32_TM_ECHOED_VOICE
	const EVENT_ROUTE_34_TM_MIRROR_MOVE
	const EVENT_ROUTE_39_TM_ZEN_HEADBUTT
	const EVENT_OLIVINE_CITY_TM_CHARGE_BEAM
	const EVENT_ROUTE_40_TM_WATER_PULSE
	const EVENT_SILVER_CAVE_OUTSIDE_REAPER_CLOTH
	const EVENT_ROUTE_28_TM_PSYSHOCK
	const EVENT_ROUTE_36_HYPER_POTION
	const EVENT_LAKE_OF_RAGE_RED_FLUTE
; New Kanto items
	const EVENT_VICTORY_ROAD_TM_DARK_PULSE
	const EVENT_MT_MOON_SQUARE_TM_DAZZLING_GLEAM
	const EVENT_ROUTE_4_TM_LASER_FOCUS
	const EVENT_ROUTE_6_TM_X_SCISSOR
	const EVENT_ROUTE_7_TM_FACADE
	const EVENT_ROUTE_8_TM_HONE_CLAWS
	const EVENT_ROUTE_9_TM_FLASH_CANNON
	const EVENT_ROUTE_10_SOUTH_TM_ROCK_POLISH
	const EVENT_ROUTE_11_TM_GRASS_KNOT
	const EVENT_ROUTE_14_TM_UPROAR
	const EVENT_FUCHSIA_CITY_TM_EARTH_POWER
	const EVENT_ROUTE_19_TM_PSYSHOCK
	const EVENT_SEAFOAM_ISLAND_B4F_TM_ICE_BEAM
	const EVENT_VIRIDIAN_FOREST_RAZOR_FANG
	const EVENT_VIRIDIAN_FOREST_BLUE_FLUTE
	const EVENT_ROUTE_27_NUGGET
	const EVENT_ROUTE_9_LIGHT_CLAY
	const EVENT_ROUTE_9_MAX_POTION
	const EVENT_ROUTE_13_OVAL_STONE
	const EVENT_SEAFOAM_ISLAND_B1F_RAZOR_CLAW
	const EVENT_SEAFOAM_ISLAND_B1F_ICE_HEAL
	const EVENT_SEAFOAM_ISLAND_B2F_GRIP_CLAW
	const EVENT_SEAFOAM_ISLAND_B3F_FULL_RESTORE
	const EVENT_SEAFOAM_ISLAND_B3F_ICE_STONE
	const EVENT_SEAFOAM_ISLAND_B4F_MAX_ELIXER
	const EVENT_GOT_BLACK_AUGURITE
; New Johto hidden items
	const EVENT_NEW_BARK_TOWN_HIDDEN_POTION
	const EVENT_DARK_CAVE_VIOLET_ENTRANCE_HIDDEN_POKE_BALL
	const EVENT_DARK_CAVE_BLACKTHORN_ENTRANCE_HIDDEN_ELIXER
	const EVENT_DARK_CAVE_BLACKTHORN_ENTRANCE_HIDDEN_MAX_REVIVE
	const EVENT_DARK_CAVE_BLACKTHORN_ENTRANCE_HIDDEN_HYPER_POTION
	const EVENT_VIOLET_CITY_HIDDEN_POKE_BALL
	const EVENT_ROUTE_32_HIDDEN_HEART_SCALE
	const EVENT_RUINS_OF_ALPH_OUTSIDE_HIDDEN_GREAT_BALL
	const EVENT_RUINS_OF_ALPH_OUTSIDE_HIDDEN_TINYMUSHROOM
	const EVENT_RUINS_OF_ALPH_OUTSIDE_HIDDEN_BIG_MUSHROOM
	const EVENT_RUINS_OF_ALPH_OUTSIDE_HIDDEN_NUGGET
	...
	const EVENT_UNION_CAVE_B1F_HIDDEN_X_SPEED
	...
	const EVENT_SLOWPOKE_WELL_B1F_HIDDEN_HEAVY_BALL
	const EVENT_SLOWPOKE_WELL_B1F_HIDDEN_FULL_HEAL
	const EVENT_ILEX_FOREST_HIDDEN_TINYMUSHROOM_1
	const EVENT_ILEX_FOREST_HIDDEN_TINYMUSHROOM_2
	const EVENT_GOLDENROD_CITY_HIDDEN_SUPER_POTION
	const EVENT_GOLDENROD_DEPT_STORE_B1F_HIDDEN_ANTIDOTE
	const EVENT_GOLDENROD_DEPT_STORE_B1F_HIDDEN_SUPER_POTION
	const EVENT_GOLDENROD_DEPT_STORE_B1F_HIDDEN_PARLYZ_HEAL
	const EVENT_GOLDENROD_UNDERGROUND_WAREHOUSE_HIDDEN_MAX_POTION
	const EVENT_GOLDENROD_UNDERGROUND_WAREHOUSE_HIDDEN_REVIVE
	const EVENT_ROUTE_35_HIDDEN_NUGGET
	const EVENT_ROUTE_35_HIDDEN_PARLYZ_HEAL
	const EVENT_NATIONAL_PARK_HIDDEN_FULL_HEAL_2
	const EVENT_ROUTE_35_HIDDEN_AWAKENING
	...
	const EVENT_ECRUTEAK_CITY_HIDDEN_GOLD_LEAF_1
	const EVENT_ECRUTEAK_CITY_HIDDEN_GOLD_LEAF_2
	const EVENT_ECRUTEAK_CITY_HIDDEN_GOLD_LEAF_3
	const EVENT_BURNED_TOWER_1F_HIDDEN_ANTIDOTE
	const EVENT_BURNED_TOWER_B1F_HIDDEN_REVIVE
	const EVENT_BURNED_TOWER_B1F_HIDDEN_NUGGET
	const EVENT_BURNED_TOWER_B1F_HIDDEN_ULTRA_BALL
	const EVENT_ROUTE_38_HIDDEN_MAX_POTION
	const EVENT_ROUTE_39_HIDDEN_X_ATTACK
	const EVENT_ROUTE_39_HIDDEN_PP_UP
	const EVENT_OLIVINE_CITY_HIDDEN_FULL_HEAL
	const EVENT_OLIVINE_CITY_HIDDEN_PEARL
	const EVENT_ROUTE_41_HIDDEN_STARDUST_1
	const EVENT_ROUTE_41_HIDDEN_STARDUST_2
	const EVENT_ROUTE_41_HIDDEN_STARDUST_3
	const EVENT_ROUTE_41_HIDDEN_STARDUST_4
	const EVENT_WHIRL_ISLAND_B1F_HIDDEN_REVIVE
	const EVENT_WHIRL_ISLAND_B1F_HIDDEN_PEARL
	const EVENT_WHIRL_ISLAND_B1F_HIDDEN_SUPER_REPEL
	const EVENT_CIANWOOD_CITY_HIDDEN_STARDUST
	const EVENT_MOUNT_MORTAR_1F_OUTSIDE_HIDDEN_REVIVE
	const EVENT_MOUNT_MORTAR_1F_INSIDE_HIDDEN_ETHER
	const EVENT_MOUNT_MORTAR_1F_INSIDE_HIDDEN_NUGGET
	const EVENT_MOUNT_MORTAR_1F_INSIDE_HIDDEN_RARE_CANDY
	const EVENT_MOUNT_MORTAR_2F_INSIDE_HIDDEN_HYPER_POTION
	const EVENT_MOUNT_MORTAR_2F_INSIDE_HIDDEN_HP_UP
	const EVENT_MOUNT_MORTAR_2F_INSIDE_HIDDEN_FULL_HEAL
	const EVENT_MOUNT_MORTAR_B1F_HIDDEN_ULTRA_BALL
	const EVENT_TEAM_ROCKET_BASE_B2F_HIDDEN_X_SP_DEF
	const EVENT_LAKE_OF_RAGE_HIDDEN_MAX_REVIVE
	const EVENT_ICE_PATH_1F_HIDDEN_ICE_HEAL
	const EVENT_ICE_PATH_B1F_HIDDEN_REVIVE
; New Kanto hidden items
	const EVENT_ROUTE_27_HIDDEN_BIG_MUSHROOM
	const EVENT_ROUTE_26_HIDDEN_TINYMUSHROOM
	const EVENT_VICTORY_ROAD_HIDDEN_PP_UP
	const EVENT_VICTORY_ROAD_HIDDEN_ULTRA_BALL
	const EVENT_VICTORY_ROAD_HIDDEN_MAX_REVIVE
	const EVENT_VICTORY_ROAD_HIDDEN_HYPER_POTION
	const EVENT_VICTORY_ROAD_HIDDEN_ZINC
	const EVENT_VIRIDIAN_CITY_HIDDEN_NUGGET
	const EVENT_VIRIDIAN_FOREST_HIDDEN_TINYMUSHROOM_1
	const EVENT_VIRIDIAN_FOREST_HIDDEN_TINYMUSHROOM_2
	const EVENT_VIRIDIAN_FOREST_HIDDEN_TINYMUSHROOM_3
	const EVENT_DIGLETTS_CAVE_HIDDEN_CALCIUM
	const EVENT_PEWTER_CITY_HIDDEN_GUARD_SPEC
	const EVENT_PEWTER_CITY_HIDDEN_MAX_REVIVE
	const EVENT_PEWTER_CITY_HIDDEN_PP_UP
	const EVENT_ROUTE_3_HIDDEN_HYPER_POTION
	const EVENT_ROUTE_3_HIDDEN_STAR_PIECE
	const EVENT_MOUNT_MOON_HIDDEN_REVIVE
	const EVENT_ROUTE_4_HIDDEN_BIG_MUSHROOM
	const EVENT_VERMILION_CITY_HIDDEN_HEART_SCALE
	const EVENT_VERMILION_CITY_HIDDEN_PEARL
	const EVENT_VERMILION_CITY_HIDDEN_BIG_PEARL
	const EVENT_ROUTE_10_HIDDEN_CARBOS
	const EVENT_ROCK_TUNNEL_B1F_HIDDEN_REVIVE
	const EVENT_ROUTE_18_HIDDEN_ELIXIR
	const EVENT_FUCHSIA_CITY_HIDDEN_NUGGET
	const EVENT_FUCHSIA_CITY_HIDDEN_BIG_MUSHROOM
	const EVENT_FUCHSIA_CITY_HIDDEN_MAX_REVIVE
	const EVENT_ROUTE_19_HIDDEN_REVIVE
	const EVENT_ROUTE_19_HIDDEN_MAX_ELIXIR
	...
	const EVENT_ROUTE_20_HIDDEN_SEA_SCALE
	const EVENT_ROUTE_20_HIDDEN_RAZOR_FANG
	const EVENT_CINNABAR_ISLAND_HIDDEN_IRON
	const EVENT_ROUTE_21_HIDDEN_STARDUST_1
	const EVENT_ROUTE_21_HIDDEN_STARDUST_2
	const EVENT_SEAFOAM_ISLAND_B1F_HIDDEN_ICE_HEAL
	const EVENT_SEAFOAM_ISLAND_B2F_HIDDEN_PEARL
	const EVENT_SEAFOAM_ISLAND_B2F_HIDDEN_BIG_PEARL
	const EVENT_SEAFOAM_ISLAND_B3F_HIDDEN_MAX_REVIVE
	const EVENT_SEAFOAM_ISLAND_B3F_HIDDEN_RARE_CANDY
	const EVENT_SEAFOAM_ISLAND_B4F_HIDDEN_ZINC
; New Kanto gym leader TMs
	const EVENT_GOT_TM80_ROCK_SLIDE
	const EVENT_GOT_TM55_WATER_PULSE
	const EVENT_GOT_TM24_THUNDERBOLT
	const EVENT_GOT_TM04_CALM_MIND
	const EVENT_GOT_TM35_FLAMETHROWER
	const EVENT_GOT_TM92_TRICK_ROOM
; New legendary encounters
	const EVENT_SEAFOAM_ISLAND_B4F_ARTICUNO
	const EVENT_FOUGHT_ARTICUNO
	const EVENT_CERULEAN_CAVE_B1F_MEWTWO
	const EVENT_FOUGHT_MEWTWO
; ???
	const EVENT_BEAT_COOLTRAINERF_LISA
	const EVENT_BEAT_BLACKBELT_DAISUKE
	const EVENT_BEAT_HIKER_DALE
	const EVENT_1CE
	const EVENT_1CF
	const EVENT_1D0
	const EVENT_1D1
...
; More Johto story events
	...
	const EVENT_SHOWED_VULPIX_TO_BILLS_GRANDPA
	...
	const EVENT_GOT_ICE_STONE_FROM_BILLS_GRANDPA
...
; Rematches
	const EVENT_BEAT_KIMONO_GIRL_NAOKO_REMATCH
	const EVENT_BEAT_KIMONO_GIRL_SAYO_REMATCH
	const EVENT_BEAT_KIMONO_GIRL_ZUKI_REMATCH
	const EVENT_BEAT_KIMONO_GIRL_KUNI_REMATCH
	const EVENT_BEAT_KIMONO_GIRL_MIKI_REMATCH
	...
	const EVENT_AZALEA_GYM_BUGSY
	const EVENT_GOLDENROD_GYM_WHITNEY
	const EVENT_BLACKTHORN_GYM_CLAIR
	const EVENT_MAHOGANY_GYM_PRYCE
	const EVENT_ECRUTEAK_GYM_MORTY
; ???
	const EVENT_5DF
	const EVENT_5E0
	const EVENT_5E1
...
; Kanto itemballs
	...
	const EVENT_ROUTE_4_ELECTIRIZER
